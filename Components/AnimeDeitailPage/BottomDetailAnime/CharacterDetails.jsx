...
import { EachCharacter } from "./CharacterDetails/EachCharacter";
...
export const CharacterDetails = memo(function CharacterDetails(){
  ...
  return (
   <>
     ...
     <Spacer/>
     <FlatList showsHorizontalScrollIndicator={false} contentContainerStyle={{gap:10, paddingHorizontal:10}} horizontal={true} data={characters} renderItem={({item, index})=><EachCharacter key={item?.id} image={item?.image} name={item?.name} role={item?.role} voiceActors={item?.voiceActors}/>} ListEmptyComponent={
